...
print popt, pcov
nx = np.linspace(-1, 1, num = 500)
ny = sigmoid(nx, *popt)
ax.plot(nx, ny, 'k-', label='fit')
bias = popt[0]
bw = np.log(popt[1]) * 200

ax.text(0.5, 0.1, 'bias = %s' %bias)
ax.text(0.5, 0.2, 'binding window = %s' %bw)
...
import time
fig.savefig(fig_dir + '/' + str(time.clock()) + '_ITD.png')
